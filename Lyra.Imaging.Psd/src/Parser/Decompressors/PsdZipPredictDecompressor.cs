...
internal class PsdZipPredictDecompressor : PsdDecompressorBase
{
    public override CompressionType Compression => CompressionType.ZipPredict;
    ...
    protected override void Decompress8(PsdBigEndianReader reader, ImageFrame<Rgba32> frame, FileHeader header, CancellationToken ct)
    {
        ...
        // PSD ZIP+Predict uses zlib stream
        ...
        try
        {
            for (var channel = 0; channel < channelsToRead; channel++)
            {
                for (var y = 0; y < height; y++)
                {
                    ...
                    // Undo "predictor" (delta) for 8-bit: row[x] += row[x-1]
                    UndoPredictor8(rowBuf, width);
                    ...
                    {
                        case 0:
                            for (var x = 0; x < width; x++) dst[x].R = rowBuf[x];
                            break;
                        case 1:
                            for (var x = 0; x < width; x++) dst[x].G = rowBuf[x];
                            break;
                        case 2:
                            for (var x = 0; x < width; x++) dst[x].B = rowBuf[x];
                            break;
                        case 3:
                            for (var x = 0; x < width; x++) dst[x].A = rowBuf[x];
                            break;
                    }
                }
            }
    ...
    private static void UndoPredictor8(byte[] row, int width)
    {
        // row[0] stays as-is
